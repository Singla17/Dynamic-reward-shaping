        ...
        for episode_idx in tqdm(range(num_episodes)):
            ...
            while not done:
                ...
                if user_reward_fn is not None:
                    reward = user_reward_fn(state=state, action=action.item(), new_state=new_state, a=a.var_value, b=b.var_value, c=c.var_value, d=d.var_value)
...
def run_episode(env, ppo_agent, render=True, jupyter=False, canvas=None, reward_fn=None):
    ...
    while not done:
        ...
        if reward_fn is not None:
            reward = reward_fn(state=state_as_arr, action=action.item(), new_state=new_state, a=a.var_value, b=b.var_value, c=c.var_value, d=d.var_value)
...
def main():
    ...
    ppo_agent, reward_list, fitness_list = train_ppo_agent(env=env,
                                      hyper_params=hyper_params,
                                      device=device,
                                      user_reward_fn=tunable_reward_fn)
    np.save('reward_list.npy', reward_list)
    np.save('fitness_list.npy', fitness_list)
